...
www = 'www'
commonDomains = [".com", ".net", ".org", ".info", ".gov", ".edu"]
interDomains = [".ac", ".ad", ".ae", ".af", ".ag", ".ai", ".al", ".am", ".ao", ".aq", ".ar", ".as", ".at",
...
print("Input a Page URL:\n")
url = input()
hasWWWW = len(re.findall(www, url))
for domain in commonDomains:
    result = re.findall(domain, url)
    if len(result) > 0 :
        domainMatches.append(result)
if len(domainMatches) < 1:
    for domain in interDomains:
        result = re.findall(domain, url)
        if len(result) > 0:
            domainMatches.append(result)
